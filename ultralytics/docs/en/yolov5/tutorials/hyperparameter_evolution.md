...
```bash
python train.py --epochs 10 --data data.yaml --weights yolov5s.pt --cache
```
...
for i in 0 1 2 3 4 5 6 7; do
  ...
  echo 'Starting GPU '$i'...' &&
  nohup python train.py --epochs 10 --data data.yaml --weights yolov5s.pt --cache --device $i --evolve > evolve_gpu_$i.log &
done
